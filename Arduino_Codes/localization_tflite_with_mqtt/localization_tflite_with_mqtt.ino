...
//MQTT Server Details
// MQTT Broker Info
const char* mqtt_server   = "a2b6b9feb01b4f678b5891f1f5720153.s1.eu.hivemq.cloud";   // example: xxx.s1.eu.hivemq.cloud
const int   mqtt_port     = 8883;
const char* mqtt_user     = "pulkit_IoT";
const char* mqtt_pass     = "pulkit_IoT123";
...
void setup_wifi() {
  ...
  WiFi.begin("yourssid", "yourpasswd");
...
int run_activity()
{
  ...
  if (millis() - last_imu_time >= IMU_PERIOD_MS){
    ...
    if(!window_filled) return last_prediction;
...
void loop(){
  ...
  int activity_result = run_activity();
  ...
  if(millis() - last_wifi_scan >= WIFI_PERIOD_MS)
  {
    ...
    Serial.println(act_labels[activity_result]);

    StaticJsonDocument<200> doc;

    doc["location"] = mlp_label_map[best_location];   // from run_localization()
    doc["activity"] = act_labels[activity_result];

    char payload[200];
    serializeJson(doc, payload);

    mqttClient.publish("esp32/localization", payload);
